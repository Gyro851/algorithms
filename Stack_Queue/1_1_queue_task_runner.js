...
class Runner {
  ...
  push(task) {
    ...
    if (this.tasks.runCount < this.tasks.max) {
      const _callbackDone = () => {
        this.tasks.runCount--;
        if (this.tasks.runCount < this.tasks.max && this.tasks.queue.length > 0) {
...
console.log(
  'It should execute: Task 1 STARTED -> Task 2 STARTED -> Task 3 STARTED -> Task 1..3 FINISHED -> Task 4..6 STARTED and etc'
);
